...
export default function AlertNewPostForm() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await  axios.post('/api/alert', sendData)
      .then((response) => {
        enqueueSnackbar('Alert Sent successfully!');
        reset();
        // handleClosePreview();
        setTimeout(()=>{
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <Card sx={{ p: 3 }}>
              <Stack spacing={3}>
                <RHFTextField name="title" label="Alert Title" />
                <div>
